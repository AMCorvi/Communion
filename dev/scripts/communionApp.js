...
app.factory("soundcloud", [
  ...
  function($http) {
    return $http
      .get(
        "http://api.soundcloud.com/users/829120/tracks?client_id=6GTzygpXw9qubWUmlmh70ZUHLidrgGJw"
      )
      .then(
...
app.controller("soundCtrl", [
  "$scope",
  "soundcloud",
  "$http",
  function($scope, soundcloud, $http) {
    $scope.changeTrack = function(url, image) {
      var music = document.getElementById("music");
      var playerImage = document.getElementById("imgs");
      $http.get(`${url}?client_id=6GTzygpXw9qubWUmlmh70ZUHLidrgGJw`).then(
        function(response) {
      ...
      music.setAttribute(
        "src",
        `${url}?client_id=6GTzygpXw9qubWUmlmh70ZUHLidrgGJw`
      );
